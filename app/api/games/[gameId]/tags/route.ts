...
export default function GameTagPage() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      // 2. Load the roster using InsightFC's current players API.
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 card space-y-5">
          ...
          <div className="pt-4 border-t border-zinc-800 space-y-3">
            ...
            {tags.length === 0 ? (
              <p className="muted">
                No tags yet. Click a tag button or use
                shortcuts.
              </p>
            ) : (
              <div className="space-y-2 max-h-[320px] overflow-auto pr-2">
                {tags
                  ...
                  .map((tag) => {
                    ...
                    const player = tag.player_id
                      ? roster.find(
                          (item) =>
                            item.id === tag.player_id
                        )
                      : null;
      ...
      <div className="text-xs text-zinc-500">
        Manual tagging is connected. Video tagging will be
        added next.
      </div>
    </div>
  );
